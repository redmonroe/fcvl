...
def merchants_pdf(file1=None, start_string=None, end_string=None, start_idx=0, end_idx=0):
...
def qb_extract_p_and_l(filename, keyword=None, path=None):
...
def qb_extract_security_deposit(filename, path=None):
...
def qb_extract_deposit_detail(filename, path=None):
    abs_file_path = os.path.join(path, filename)
...
def nbofi_pdf_extract_hap(filename, path=None):
