...
class Reconciler:
    ...
    def master_sum_from_payments_totaler(ten_payments=None, non_ten_pay=None, delete_mentries=None, period=None):
        try:
            if delete_mentries:
                ...
                print(
                    f'Reconciling prior to StatusObject write for period {period}: tenant payments: {ten_payments} nontenant payments: {non_ten_pay} less deleted manual entries: {delete_mentries} = {sum_from_payments}.')

            else:
                sum_from_payments = ten_payments + non_ten_pay
                print(
                    f'Reconciling prior to StatusObject write for period {period}: tenant payments: {ten_payments} nontenant payments: {non_ten_pay} = {sum_from_payments}.')
    ...
    def iter_build_assert_scrape_total_match_deposits(scrape_deposit_sum=None, grand_total_from_deposits=None, genus=None, period=None):
        try:
            ...
            print(
                f'Final check for {genus} if payment reports == scrape for {period}:')
            ...
            print(
                f'{genus}:{scrape_deposit_sum} does not equal payment report:{grand_total_from_deposits}.\n')
            raise
    ...
    def month_sheet_final_check(onesite_total=None, nbofi_total=None, period=None, genus=None):
        try:
            ...
            print(
                f'Final check for {genus} if payment reports == bank deposits {period}:')
            ...
            print(
                f'\nAssertionError in MonthSheet {genus} deposits do not match payments report for period {period}.')
            print(
                f'{genus}:{onesite_total[0][0]} does not equal payment report:{nbofi_total[0][0]}.\n')
            raise
    ...
    def backend_processing_layer_assert_bank_deposits_tenant_deposits(bank_deposits=None, sum_from_payments_report=None, period=None, genus=None, **kwargs):
        try:
            ...
            print(
                f'Reconciling {genus} deposits to payments report for {period}:')
            ...
            print(
                f'{genus}:{bank_deposits} does not equal payment report:{sum_from_payments_report}.\n')
            breakpoint()
            raise
    ...
    def findexer_assert_scrape_catches_all_target_txns(period=None, sum_of_parts=None, check=None):
        try:
            assert float(Utils.decimalconv(
                str(sum_of_parts))) == round(check, 2)
            print(
    ...
    def findex_reconcile_onesite_deposits_to_scrape_or_oc(self):
        ...
        scrape_match = self.findex_iteration_helper(
            list1=deposits_xls, list2=scrapes, target_str='0', fill_str='empty')
        opcash_match = self.findex_iteration_helper(
            list1=deposits_xls, list2=opcashes, target_str='0', fill_str='empty')
        ...
        self.findex_reconcile_helper_writer(
            list1=scrape_only, recon_str='scrape_only')
        self.findex_reconcile_helper_writer(
            list1=opcash_only, recon_str='opcash_only')
        self.findex_reconcile_helper_writer(
            list1=dep_xls_prob, recon_str='dep_prob')
