...
class Letters():
    ...
    def get_addresses(self):
        ...
        units = [(row.unit_name, row.tenant, row.last_occupied)
                 for row in Unit.select().namedtuples()]
        ...
        if units == []:
            ...
            for unit in units:
                if unit[0].split('-')[0] == 'CD':
                    addresses.append(self.fix_name(
                        unit, address_bp=Config.ADDRESS_CD))
                elif unit[0].split('-')[0] == 'PT':
                    addresses.append(self.fix_name(
                        unit, address_bp=Config.ADDRESS_PT))
        return addresses
    ...
    def get_bal_let_parameters(self, balance_list=None):
        ...
        for record in balance_letters:
            formatted_name = [name.rstrip().lstrip().capitalize()
                              for name in record.tenant_name.split(',')]
    ...
    def get_all_archived_work_orders(self):
        ...
        values = gc.broad_get(
            service=self.service, spreadsheet_id=self.spreadsheet_id, range=self.work_order_range)
    ...
    def bal_let_pprint_parameters(self, parameters):
        print('current date',
              parameters['current_date'], parameters['display_year'])
        print('display_month', parameters['display_month'])

        bal_data = list(
            zip(parameters['unit'], parameters['name'], parameters['bal_due']))
        print('\n')
    ...
    def balance_letters_to_google(self):
        ...
        db.connect()
        player = ProcessingLayer()
        parameters = self.get_bal_let_parameters(
            balance_list=player.show_balance_letter_list_mr_reconciled())
        self.bal_let_pprint_parameters(parameters)
        choice = str(
            input('Send these results to google script & make balance letters? y/n '))
        if choice == 'y':
            Letters.run_script(service=oauth(Config.my_scopes, 'script'),
                               deploy_id=Config.BALANCE_LETTER_DEPLOY_ID, function_name='balanceLetter', parameters=parameters)
        else:
    ...
    def get_prev_daterange_by_month(self, year, month, prev):
        ...
        for i in range(prev):
            ...
            since.append(datetime.strptime(
                str(date(year, month, 1)), "%Y-%m-%d"))
            till.append(datetime.strptime(
                str(date(year, month, num_days)), "%Y-%m-%d"))
            month -= 1
        return since, till
    ...
    def rent_receipts_plus_balance(self):
        ...
        rent_roll = query.get_rent_roll_by_month_at_first_of_month(
            first_dt=first_dt, last_dt=last_dts)
        ...
        first_date_of_months, last_date_of_months = self.get_prev_daterange_by_month(
            2022, 7, lookback_months)
        ...
        total_payments = query.get_payments_by_tenant_by_period(
            first_dt=earliest_date_in_range, last_dt=last_date_in_range, cumsum=True)

        all_payments = query.        get_payments_by_tenant_by_period(
            first_dt=earliest_date_in_range, last_dt=last_date_in_range)
    ...
    def check_rs_status(self):
        titles_dict = Utils.get_existing_sheets(
            oauth(Config.my_scopes, 'sheet'), Config.TEST_RS)
        idx_list = Utils.existing_ids(
            {name: id2 for name, id2 in titles_dict.items() if name != 'intake'})
        return idx_list

    def rent_receipts_configuration(self):
        idx_list = self.check_rs_status()
        sheet_choice = idx_list[int(
            input('Please select a sheet to make receipts from: '))]
        parameters = {
            'current_date': datetime.strftime(datetime.utcnow(), '%Y-%m-%d'),
...
class DocxWriter(Letters):
    ...
    def insert_header(self, document=None):
        paragraph = document.add_paragraph(
            'Fall Creek Village I', style='No Spacing')
        paragraph.paragraph_format.left_indent = Inches(self.header_indent)
        paragraph = document.add_paragraph(
            '3515 N. Pennsylvania St.', style='No Spacing')
        paragraph.paragraph_format.left_indent = Inches(self.header_indent)
        paragraph = document.add_paragraph(
            'Indianapolis, IN 46205', style='No Spacing')
        paragraph.paragraph_format.left_indent = Inches(self.header_indent)
        paragraph = document.add_paragraph(
            '(317) 925-5558', style='No Spacing')
        paragraph.paragraph_format.left_indent = Inches(self.header_indent)
        paragraph = document.add_paragraph(
            'TTY: 711 or (800) 743-3333', style='No Spacing')
        paragraph.paragraph_format.left_indent = Inches(self.header_indent)
        paragraph = document.add_paragraph(
            '                                                                                                                                               ')
    ...
    def format_docx_rent_receipt(self, document=None, parameters=None, r_recs=None):
        for address in r_recs:
            ...
            paragraph = document.add_paragraph(
                city_and_state, style='No Spacing')
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph(
                f'Dear {address[0]},', style='No Spacing')
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph('Thank you for your rent payment for ' +
                                               parameters['display_month'] + ' ' + Config.current_year + '.', style='No Spacing')
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph(
                'Our records show that you paid: $ ' + address[5] + '.', style='No Spacing')
    ...
    def format_workorders(self, document=None, parameters=None, records=None):
        for record in records:
            ...
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph(
                f'Work Order Date: {record[0]}', style='No Spacing')
            paragraph = document.add_paragraph(' ', style='No Spacing')
            paragraph = document.add_paragraph(
                f'Tenant Name: {record[1]}', style='No Spacing')
            paragraph = document.add_paragraph(
    ...
    def export_workorders_to_docx(self, first_dt=None, last_dt=None):
        ...
        document = self.format_workorders(
    ...
    def docx_rent_receipts_from_rent_sheet(self, mode=None):
        ...
        if mode == 'testing':
            parameters = {'current_date': '2022-10-27', 'display_month': 'March',
                          'sheet_choice': '2022-03', 'rent_sheet': '1Z_Qoz-4ehalutipyH2Vj5k-y2b78U69Bc7uXoBKK47Q'}
        else:
            parameters = self.rent_receipts_configuration()

        rs_name_pay = [(self.fix_name2(row[0]), row[9]) for row in self.load_from_sheet(
            parameters['sheet_choice'], parameters['rent_sheet'])]
        ...
        for name, pay in rs_name_pay:
            for address in self.get_addresses():
                if name == address[0]:
                    r_recs.append(
                        [address[0], address[1], address[2], address[3], address[4], pay])
                    sum_for_test.append(pay)

        document = self.format_docx_rent_receipt(
            document=document, parameters=parameters, r_recs=r_recs)

        if mode == 'testing':
            document.core_properties.title = 'docx_rent_receipts'
            save_path = self.testing_save_path / Path(self.testing_save_name)
        else:
            save_name = 'rent_receipts_' + \
                parameters['current_date'] + '_' + \
                parameters['sheet_choice'] + '.docx'
